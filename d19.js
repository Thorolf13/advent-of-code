const input = `fxg{x<2288:tn,m<3059:mlg,xl}
...
workflows = workflows.map(line =>{
  ...
  conditions = conditions.slice(0, -1).split(',').map(x=>{
...
parts = parts.map(line => {
  let obj
  eval('obj='+line.replace(/=/g, ':'))
  obj.currentWorkflow = 'in'
  return obj
})
...
while(parts.length){
  ...
  const { currentWorkflow, x, m, a, s } = part
  const workflow = workflows.find(w => w.name === currentWorkflow)
  const {conditions} = workflow

  for( const condition of conditions ){
    const test = eval(condition[0])
    if( test ){
      const dest = condition[1]

      if( dest === 'A' ) acceps.push(part)
      else if( dest === 'R' ) rejects.push(part)
      else {
        part.currentWorkflow = dest
        parts.push(part)
      }
...
// console.log(acceps, rejects)

const res = acceps.reduce((acc, part) => acc + part.s + part.m + part.a + part.x, 0)
...
function copyRanges(ranges){
  return {a:[...ranges.a], m:[...ranges.m], s:[...ranges.s], x:[...ranges.x]}
}

function cutRanges(ranges, test){  
  const match = test.match(/([amsx])([<>])(\d+)/)
  if( !match ) return [ranges]

  const range1 = copyRanges(ranges)
  const range2 = copyRanges(ranges)
  ...
  if( op === '<' ){
    range1[cat][1] = val - 1 //accepted part
    range2[cat][0] = val
  }
  else if( op === '>' ){
    range1[cat][0] = val + 1 //accepted part
    range2[cat][1] = val
  }
...
function mapRanges(ranges, workflowName){
  // console.log('mapranges', ranges, workflowName)

  if( workflowName == 'A'){
    accepsRanges.push(ranges)
    return
  }
  if( workflowName == 'R'){
    rejectsRanges.push(ranges)
    return
  }
  ...
  let leftRanges = [ranges]
  let newRanges = []

  for(const condition of conditions){
    const  [test, dest] = condition

    while(leftRanges.length){
      const r = leftRanges.shift()
      const cutted = cutRanges(r, test)

      mapRanges(cutted[0], dest)
      newRanges.push(cutted[1])      
    }

    leftRanges = newRanges
    newRanges = []
  }
}

mapRanges({a:[1, 4000], m:[1, 4000], s:[1, 4000], x:[1, 4000]}, 'in')
// console.log(accepsRanges, rejectsRanges)
const rr = accepsRanges.reduce((acc, ranges) => {
  const [a, m, s, x] = Object.values(ranges)
  return acc + BigInt(a[1] - a[0] + 1) * BigInt(m[1] - m[0] + 1) * BigInt(s[1] - s[0] + 1) * BigInt(x[1] - x[0] + 1)
}, BigInt(0))
