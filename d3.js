let input = `.........798...145.........629....579.....455.....................130.............243.................154........167........................
...
function isNearSymbol(x, y1, y2){
  for(a=x-1;a<=x+1;a++){
    for(b=y1-1;b<=y2+1;b++){
      if(arr[a] && arr[a][b] && /[^0-9\.]/.test(arr[a][b])){
        return true
      }
    }
  }
...
function getNumbersNear(x, y){
  const result = []

  for( i=-1 ; i<=1 ; i++ ){
    [...arr[x+i].matchAll(/\d+/g)].filter((match) => {
      const y1 = match.index
      const y2 = match.index + match[0].length - 1
      if (y2 >= y-1 && y1 <= y+1){
        result.push(match[0])
      }
    })
...
arr.forEach((line, index) => {
  [...line.matchAll(/\d+/g)].forEach((match) => {
    ...
    if( isNearSymbol(x, y1, y2) ){
      // console.debug('line '+index+' '+match[0], isNearSymbol(x, y1, y2))
      s += +match[0]
    }
  })
})
...
arr.forEach((line, index) => {
  [...line.matchAll(/\*/g)].forEach((match) => {
    const n = getNumbersNear(index, match.index)
