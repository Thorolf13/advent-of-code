const input = `FF-7F-7.|FJ7L-7-.FL-F7|-7FFF77J.|7F--J-FF7-L..-7FF-LF7F|-F|7-L-FLJ7-FF-JFF-7-7.-J-.--7.-F---FJ-|..-J7.|J-77-F7---77FFLJ7.LFJF7.FL|-J--7-LJ-|
...
function isConnected(value, dir){
  switch(dir){
    case 'N': return isIn(value, 'S|JL')
    case 'S': return isIn(value, 'S|F7')
    case 'E': return isIn(value, 'S-FL')
    case 'W': return isIn(value, 'S-7J')  
  }
}
...
class Grid{
    ...
    getPos(x,y,dir){
        switch(dir){
            case 'N': return isConnected(this.get(x, y-1), 'S') ? {x, y: y-1} : {x:-1, y:-1};
            case 'S': return isConnected(this.get(x, y+1), 'N') ? {x, y: y+1} : {x:-1, y:-1};
            case 'E': return isConnected(this.get(x+1, y), 'W') ? {x: x+1, y} : {x:-1, y:-1};
            case 'W': return isConnected(this.get(x-1, y), 'E') ? {x: x-1, y} : {x:-1, y:-1};
        }
    }
...
while(todo.length){
  ...
  if( isConnected(value, 'N') ) todo.push({...grid.getPos(x, y, 'N'), i: i + 1})
  if( isConnected(value, 'S') ) todo.push({...grid.getPos(x, y, 'S'), i: i + 1})
  if( isConnected(value, 'E') ) todo.push({...grid.getPos(x, y, 'E'), i: i + 1})
  if( isConnected(value, 'W') ) todo.push({...grid.getPos(x, y, 'W'), i: i + 1})



  // switch(value){    
  //   case '|' :
  //     todo.push({...grid.getPos(x, y, 'N'), i: i + 1})
  //     todo.push({...grid.getPos(x, y, 'S'), i: i + 1})
  //     break
  //   case '-' :
  //     todo.push({...grid.getPos(x, y, 'W'), i: i+1 })
  //     todo.push({...grid.getPos(x, y, 'E'), i: i+1 })
  //     break
  //   case 'L' :
  //     todo.push({...grid.getPos(x, y, 'N'), i: i+1 })
  //     todo.push({...grid.getPos(x, y, 'E'), i: i+1 })
  ...
  // todo = todo.filter(i=>i!==undefined).filter(({x, y}) => grid.get(x, y) !== undefined)
}
