let input = `Game 1: 20 green, 3 red, 2 blue; 9 red, 16 blue, 18 green; 6 blue, 19 red, 10 green; 12 red, 19 green, 11 blue
...
input.split(/[\n\r]/g).forEach((line)=>{
    ...
    for(const cube of cubes){
      ...
      for( const part of parts){
        let [nb, color] = part.split(' ')

        mini[color] = Math.max(mini[color] || 0, +nb)


        switch(color){
          case 'red':
            if(+nb > 12) possible = false
            break
          case 'green':
            if(+nb > 13) possible = false
            break
          case 'blue':
            if(+nb > 14) possible = false
            break
          default:
              possible = false
        }
    ...
    power = Object.values(mini).reduce((a,b)=>a*b, 1)
    s2 += power

    if(possible) s += parseInt(id)
})
