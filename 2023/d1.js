...
function parseDigit(str){
  const index = digits.indexOf(str)
  if(index === -1){
    return parseInt(str)
  } else{
    return index
...
input.split(/[\n\r]+/g).forEach(line=>{
  // const firstDigit = line.match(/^.*?(\d)/)[1] //part 1
  // const lastDigit = line.match(/.*(\d).*?$/)[1]
  const firstDigit = line.match(/^.*?(\d|one|two|three|four|five|six|seven|eight|nine)/)[1]
  const lastDigit = line.match(/.*(\d|one|two|three|four|five|six|seven|eight|nine).*?$/)[1]

  s += parseDigit(firstDigit) * 10 + parseDigit(lastDigit)
})
